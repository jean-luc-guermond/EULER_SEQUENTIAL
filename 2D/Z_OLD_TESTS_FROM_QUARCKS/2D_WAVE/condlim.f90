MODULE boundary_conditions
  ...
  REAL(KIND=8), PARAMETER :: pi=ACOS(-1.d0) , beta=5.d0, x0=-1, y0=0, r0_sq=16.d0, &
...
CONTAINS
  ...
  FUNCTION rho_anal(rr) RESULT(vv)
    ...
    radius_sq = (rr(1,:)-x0-u_infty*inputs%time)**2 + (rr(2,:)-y0)**2

    DO n = 1, SIZE(vv)
       IF (radius_sq(n).GE.r0_sq) THEN
          vv(n) = 1.d0 
       ELSE
          vv(n) = 1.d0 + (r0_sq-radius_sq(n))**3/r0_sq**3 
       END IF
 END DO
