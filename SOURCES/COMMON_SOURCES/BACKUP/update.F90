MODULE update_euler
  ...
  USE input_data
  USE mesh_handling
  PUBLIC:: euler, construct_matrices, compute_dt
  PRIVATE
  LOGICAL, PUBLIC                              :: plot
...
!!$  END SUBROUTINE entropy_residual_bis
  ...
  SUBROUTINE entropy_residual(un,rkgal)
    ...
    ELSE IF (inputs%entropy_type=='s') THEN
       ...
       en = un(1,:)*s  !S = rho*s
    ELSE
       WRITE(*,*) ' Bug: entropy_type not defined', inputs%entropy_type
       STOP
    END IF
  ...
  SUBROUTINE smoothness_viscosity(un)
    ...
    DO i = 1, mesh%np
       ...
       ELSE
          ...
          bbeta(i) = 0.d0
       END IF
       alpha(i) = (MAX(alpha(i)-0.25,0.d0)*4.d0/3.d0)**3 !New formulation, June 16, 2017
    END DO
...
!!$          END IF
          ...
          !   STOP
          !END if
          CALL Newton_secant(un(:,i),un(:,j),Plr,ttt,psi_entrop,psi_entrop_prime,E_small,E_small_relax)
          IF (ttt.le.0.d0) THEN
             WRITE(*,*) ' BUG ttt<0', ttt
             STOP
          END IF
          ...
          ttt = 1.d0/lambda_max
          CALL entrop_ineq(un(:,i),un(:,j),Plr,ttt,aaa_entrop,bbb_entrop,S_small,S_small_relax)
          lambda_max = MAX(lambda_max,1.d0/ttt)
          !IF (lambda_max>lambda_max_safe*(1.d0+small)) THEN
...
10        dij%aa(p) = norm_cij*lambda_max
  ...
  SUBROUTINE rho_e_limit(ui,uj,Pij,limiter,rho_e_min)
    IMPLICIT NONE
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: Pij
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: ui, uj
    REAL(KIND=8)                                 :: limiter, rho_e_min
    REAL(KIND=8), DIMENSION(k_dim+2) :: uavg
    REAL(KIND=8) :: a, b, c, delta, t1, t2

    uavg = (ui+uj)/2
    a = Pij(1)*Pij(k_dim+2) - SUM(Pij(2:k_dim+1)**2)/2.d0
    b = Pij(k_dim+2)*uavg(1) + Pij(1)*uavg(k_dim+2)-SUM(Pij(2:k_dim+1)*uavg(2:k_dim+1)) &
         - rho_e_min*Pij(1)
    c = uavg(1)*uavg(k_dim+2) - SUM(uavg(2:k_dim+1)**2)/2.d0 -  rho_e_min*uavg(1)
    ...
    IF (a.EQ.0.d0) THEN
       ...
       ELSE
          t1 = limiter
       END IF
       limiter = min(t1,limiter)
       IF (limiter.LE.0d0) THEN
          write(*,*) ' BUG rho_e_limit first if', limiter
          stop
       END IF
       RETURN
    END IF
    ...
    END IF
    !IF (limiter.LE.0d0) THEN
    !   write(*,*) ' BUG rho_e_limit exit', limiter, a, t1, t2, a*t1**2+b*t1+c, a*t2**2+b*t2+c
    !   stop
    !END IF

  END SUBROUTINE rho_e_limit

  SUBROUTINE Newton_secant(ui, uj, Pij,limiter,psi_func,psi_prime_func,psi_small,psi_relax)
    IMPLICIT NONE
    INTERFACE
    ...
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: Pij
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: ui, uj
    REAL(KIND=8), INTENT(IN)    :: psi_small, psi_relax
  ...
  SUBROUTINE entrop_ineq(ui,uj,Pij,limiter,aaa,bbb,psi_small,psi_relax)
    IMPLICIT NONE
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: Pij
    REAL(KIND=8), DIMENSION(k_dim+2), INTENT(IN) :: ui, uj
    REAL(KIND=8), INTENT(IN)    :: aaa, bbb, psi_small, psi_relax
    REAL(KIND=8), INTENT(INOUT) :: limiter
    REAL(KIND=8), DIMENSION(k_dim+2) ::  ul, ur, ulr
