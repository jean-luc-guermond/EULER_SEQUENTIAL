MODULE update_euler
  ...
  REAL(KIND=8), DIMENSION(:), POINTER          :: urelaxi, drelaxi
...
CONTAINS 
  ...
  SUBROUTINE convex_limiting_proc(un,ulow,unext)
    ...
    IF (inputs%if_relax_bounds) THEN
       ...
       CALL relax_cmin(un,ccmin,mass,drelaxi)
    END IF
    ...
    CALL compute_fct_matrix_full(inputs%dt,un,du,fctmat,dijH,dij,mc_minus_ml)
  ...
  SUBROUTINE COMPUTE_DT(u0)
    ...
    CALL compute_dij(u0)
    !===SSP 3 => factor 3
    inputs%dt = 3*inputs%CFL*0.5/MAXVAL(ABS(dij%aa(diag))/lumped)
